...
if (!class_exists('Wp_Kraken')) {
    ...
    class Wp_Kraken
    {
        ...
        public function __construct()
        {
            ...
            add_action('wp_ajax_kraken_reset', [&$this, 'kraken_media_library_reset']);
            add_action('wp_ajax_kraken_optimize', [&$this, 'kraken_optimize']);
            add_action('wp_ajax_kraken_request', [&$this, 'kraken_media_library_ajax_callback']);
            add_action('wp_ajax_kraken_reset_all', [&$this, 'kraken_media_library_reset_all']);
            ...
            if ((!empty($this->kraken_settings) && !empty($this->kraken_settings['auto_optimize'])) || !isset($this->kraken_settings['auto_optimize'])) {
                add_action('add_attachment', [&$this, 'kraken_media_uploader_callback']);
            ...
            add_action('admin_menu', [&$this, 'kraken_menu']);
        }
        ...
        public function kraken_menu()
        {
            add_options_page('Kraken Image Optimizer Settings', 'Kraken.io', 'manage_options', 'wp-krakenio', [&$this, 'kraken_settings_page']);
        }
        ...
        public function kraken_settings_page()
        {
            if (!empty($_POST)) {
            ...
            if (empty($input['api_key']) || empty($input['api_secret'])) {
                ...
                if (false !== $status) {
                    if (isset($status['active']) && true === $status['active']) {
                        if ('Developers' === $status['plan_name']) {
        ...
        {
            if ('options-media.php' == $hook || 'upload.php' == $hook || 'settings_page_wp-krakenio' == $hook) {
                wp_enqueue_script('jquery');
                if (KRAKEN_DEV_MODE === true) {
                    wp_enqueue_script('async-js', plugins_url('/js/async.js', __FILE__));
                    wp_enqueue_script('tipsy-js', plugins_url('/js/jquery.tipsy.js', __FILE__), ['jquery']);
                    wp_enqueue_script('modal-js', plugins_url('/js/jquery.modal.min.js', __FILE__), ['jquery']);
                    wp_enqueue_script('ajax-script', plugins_url('/js/ajax.js', __FILE__), ['jquery']);
                    ...
                    wp_enqueue_script('kraken-js', plugins_url('/js/dist/kraken.min.js', __FILE__), ['jquery']);
                    wp_localize_script('kraken-js', 'ajax_object', ['ajax_url' => admin_url('admin-ajax.php')]);
        ...
        public function get_api_status($api_key, $api_secret)
        {
            if (!empty($api_key) && !empty($api_secret)) {
         ...
         */
        public function kraken_media_library_ajax_callback()
        {
            ...
            if (wp_attachment_is_image($image_id)) {
                ...
                if ($optimize_main_image) {
                    ...
                    if (!$thumbs_optimized) {
                        ...
                        if (0 !== strcmp($kraked_thumbs_mode, $this->optimization_type)) {
                    ...
                    if (!empty($api_result) && !empty($api_result['success'])) {
                        ...
                        echo json_encode(['error' => $api_result['message'], '']);
                    }
                } else {
                    // get metadata for thumbnails
                    $image_data = wp_get_attachment_metadata($image_id);
        ...
        public function is_successful($response)
        {
        }
         ...
         */
        public function kraken_media_uploader_callback($image_id)
        {
            $this->id = $image_id;
            ...
            if (!$this->isApiActive()) {
                ...
                remove_action('add_attachment', [&$this, 'kraken_media_uploader_callback']);
        ...
        public function kraken_media_library_reset()
        {
            $image_id = (int) $_POST['id'];
            ...
            echo json_encode(['success' => true, 'original_size' => $original_size, 'html' => $this->optimize_button_html($image_id)]);
            wp_die();
        }

        public function kraken_media_library_reset_all()
        {
            $result = null;
        ...
        public function optimize_button_html($id)
        {
...
EOD;
        ...
        public function show_api_secret()
        {
            $settings = $this->kraken_settings;
        ...
        public function show_auto_optimize()
        {
            $options = get_option('_kraken_options');
        ...
        public function show_bulk_async_limit()
        {
            $options = get_option('_kraken_options');
        ...
        public static function KBStringToBytes($str)
        {
            $temp = floatval($str);
            $rv = false;
            if (0 == $temp) {
        ...
        public static function calculate_savings($meta)
        {
            if (isset($meta['original_size'])) {
                ...
                if (false !== stripos($saved_bytes, 'kb')) {
        ...
        public function generate_stats_summary($id)
        {
            ...
            if (isset($image_meta['original_size'])) {
                ...
                if (false !== stripos($original_size, 'kb')) {
        ...
        public function results_html($id)
        {
            $settings = $this->kraken_settings;
            $optimize_main_image = !empty($settings['optimize_main_image']);
        ...
        public function fill_media_columns($column_name, $id)
        {
            ...
            if (0 === $original_size || false === $original_size) {
                echo '0 bytes';
            ...
            if (0 === strcmp($column_name, 'original_size')) {
                if (wp_attachment_is_image($id)) {
                    ...
                    if (isset($meta['original_size'])) {
                        if (false !== stripos($meta['original_size'], 'kb')) {
            ...
            } elseif (0 === strcmp($column_name, 'kraked_size')) {
        ...
        public function optimize_image($image_path, $type, $resize = false)
        {
            ...
            if ($resize) {
                ...
                if ($width && $height) {
                    $params['resize'] = ['strategy' => 'auto', 'width' => $width, 'height' => $height];
                } elseif ($width && !$height) {
                    $params['resize'] = ['strategy' => 'landscape', 'width' => $width];
                } elseif ($height && !$width) {
                    $params['resize'] = ['strategy' => 'portrait', 'height' => $height];
                }
            }
        ...
        public function get_sizes_to_krak()
        {
            $settings = $this->kraken_settings;
            $rv = [];

            foreach ($settings as $key => $value) {
                if (0 === strpos($key, 'include_size') && !empty($value)) {
                    $rv[] = $key;
                }
            }
                ...
                foreach ($sizes as $key => $size) {
                    ...
                    if (false !== file_exists($thumb_path)) {
                        ...
                        if (!empty($result) && isset($result['success']) && isset($result['kraked_url'])) {
                            ...
                            if (0 !== (int) $result['saved_bytes']) {
                                if ($this->replace_image($thumb_path, $kraked_url)) {
                                    $this_thumb = ['thumb' => $key, 'file' => $size['file'], 'original_size' => $result['original_size'], 'kraked_size' => $result['kraked_size'], 'type' => $this->optimization_type];
                                    $thumbs_optimized_store[] = $this_thumb;
                                }
                            } else {
                                $this_thumb = ['thumb' => $key, 'file' => $size['file'], 'original_size' => $result['original_size'], 'kraked_size' => $result['original_size'], 'type' => $this->optimization_type];
                                $thumbs_optimized_store[] = $this_thumb;
                            }
                        }
                    }
